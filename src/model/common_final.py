...
class ActivatedOp(nn.Module): 
    ...
    def forward(self, op2, x):
        # x, op2 = 256
        # op2 = prune(op2, reshape(self.gate,self.input_gate))
        # x = prune(x, reshape(self.gate,self.input_gate))
        op2 = prune(op2, self.gate)
        x= prune(x, self.gate)
        # TODO is it ok to prune without regarding index??
        if self.op_type == 'skip_only':
            return op2+x
        
        elif self.op_type == 'conv_tot':
            op3 = self.conv3(op2) # 54
            # convtot_flop=torch.prod(torch.tensor(op3.shape[1:]))*18*op2.shape[1]
            op3 = op3.mul(self.res_scale)
            # skip 10
            # x = (x.clone().permute(0,2,3,1)*reshape(self.skip_gate,self.gate).cpu()).permute(0,3,1,2) # 54 (10 activated)
            x = (x.clone().permute(0,2,3,1)*reshape(self.skip_gate,self.gate)).permute(0,3,1,2) # 54 (10 activated)
            op3 += x
             
            # return op3, convtot_flop
            return op3
...
class ResBlock(nn.Module):
    def __init__(self, conv, n_feats, kernel_size,idx, input,op1,op2,P,R,T,PR,PT,RT,PRT, skip_gate_bef, resconv3_gate_bef,skip_gate,resconv3_gate, resconv2_gate,bias=True, bn=False, res_scale=1):
        ...
        # self.ch_length = len(P)+len(R)+len(T)+len(PR)+len(PT)+len(RT)+len(PRT)+len(NOP) 
        ...
        # self.actNOP = Activation(act_index=NOP, act_type='nop')
        ...
        # print(resconv2_gate.sum()) #141
        # print(self.skip_only_gate.sum()) #41
        # print(self.conv_only_gate.sum()) #44
        # print(self.skip_conv_gate.sum()) #10
    ...
    def forward(self, x):
        ...
        x= x.cuda()
        # print(x.shape)
        # print(self.skip_gate_bef.sum())
        # print(self.skip_gate.sum())

        if self.idx == 0:
            x_conv2 = prune(x, self.skip_gate) # activate 51
        else:
            x_conv2 = prune_n(x, int(self.skip_gate.sum())) # just activate first  95 / ...
        # CONSIDER INDEX!
        # 2nd Resblock : front 95 is activated / back 256-95 is deactivated
        # x_conv2 = prune_n(x, int(self.skip_gate.sum())) # just activate first 51 / 95 / ...

        # reshaped_skip_gate=reshape(self.skip_gate, self.input_gate)
        # else, just use x as x_conv2
        # x_conv2 = prune(x,reshaped_skip_gate) # 51 out of x(input=before output) : 256, 95, 95+34, ..
        # x_conv2 = prune(x,self.skip_gate_bef) # 51 out of 256
        # print(x_conv2.shape)
        # print("x_conv2",x_conv2.shape )
        op2 = self.conv2(x_conv2) # 51 -> 141
        ### flops+=torch.prod(torch.tensor(op2.shape[1:]))*18*x_conv2.shape[1]
        
        # print("op2",op2.shape)
        # print("nop",prune(x,1-self.resconv2_gate).shape) # 115
        # op2 = torch.cat([self.actP(op2), self.actR(op2), self.actT(op2), self.actPR(op2), self.actPT(op2), self.actRT(op2), self.actPRT(op2),prune(x,1-self.resconv2_gate)], dim=1)
        ...
        # op2 = torch.cat([self.actP(op2).cuda(), self.actR(op2).cuda(), self.actT(op2).cuda(),
        #                 self.actPR(op2).cuda(), self.actPT(op2).cuda(), self.actRT(op2).cuda(), 
        #                 self.actPRT(op2).cuda(), prune(x,1-self.op2).cuda()], dim=1) # 141 + 115 = 256
        # op2 = torch.cat([op2_act, reshape(x,1-self.op2)],dim=1)# 141+115=256
        # assume that conv index is not impacted by channel order
        # op2 front 141: activated / back 115 : de-activated
        
        # op3 = torch.cat([self.skip_only(op2,x).cuda(), self.conv_only(op2,x).cuda(), self.skip_conv(op2,x).cuda()],dim=1)
        # op3 = torch.cat([self.skip_only(op2,x), self.conv_only(op2,x), self.skip_conv(op2,x)],dim=1)
        # op3 = torch.cat([self.skip_only(op2,x).cuda(), self.conv_only(op2,x)[0].cuda(), self.skip_conv(op2,x)[0].cuda()],dim=1)
        # flops+= self.conv_only(op2,x)[1]+self.skip_conv(op2,x)[1]

        op3_gate = self.skip_only_gate + self.conv_only_gate+self.skip_conv_gate 
        # print("op2_tot",op2.shape)
        # print("x",x.shape)
        # print("skip_only",self.skip_only(op2,x).shape) 
        # print("conv_only",self.conv_tot(op2,x)[0].shape )
        # print("nothing", prune(x,1-op3_gate).shape)

        # op3 = torch.cat([self.skip_only(op2,x), self.conv_tot(op2,x)[0],prune(x,1-op3_gate)],dim=1)
        # op3 = torch.cat([self.skip_only(op2,x).cuda(), self.conv_tot(op2,x)[0].cuda(),prune(x,1-op3_gate)],dim=1)
        op3 = torch.cat([self.skip_only(op2,x).cuda(), self.conv_tot(op2,x).cuda(),prune(x,1-op3_gate)],dim=1)
        # op3 = torch.cat([self.skip_only(op2,x).cuda(), self.conv_tot(op2,x)[0].cuda()],dim=1)
        ### flops+= self.conv_tot(op2,x)[1]
        # assume that conv index is not impacted by channel order
        # True ??? TODO
        # activated channel order has consistently changed -> so this is probably NOT true!
