    ...
    def test(self):
        ...
        for idx_scale, scale in enumerate(self.scale):
            ...
            for i in range(20):
                ...
                # hr = torch.randn(input_size[0][0],input_size[0][1],input_size[0][2]*self.args.scale[0],input_size[0][3]*self.args.scale[0])
            ...
            # self.ckp.write_log('Input Image Size: {}x{}'.format(lr.size(2),lr.size(3)))
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for lr, hr, filename in tqdm(d, ncols=80):
                    i+=1
                    lr, hr = self.prepare(lr, hr)
                    input_size.append(lr.shape)
                    #### FLOPs calculation ####
                    # self.ckp.write_log('Input Image Size: {}x{}'.format(lr.size(2),lr.size(3)))
                    # _output = (lr.size(2)*self.args.scale[0],lr.size(3)*self.args.scale[0])
                    ...
                    # torch.cuda.synchronize()
                    # toc = time.time() - tic
                    # self.ckp.write_log('Operation time: {:.3f}s'.format(toc))
            ...
            '''
            ####################### THIS FLOPS CALCULATION IS WRONG! ##########################
            _output = (1280,720)
            _input= (_output[0]//self.args.scale[0],_output[1]//self.args.scale[0])

            print(len(self.args.op1),len(self.args.op2),len(self.args.op_last))
            kernel_size = 3
            flops= (_input[0]*_input[1])*2*(kernel_size*kernel_size)*self.args.n_colors*len(self.args.op1)
            ...
            for i in range (self.args.n_resblocks):
                ...
                ch_length += len(self.args.P[i])+len(self.args.R[i])+len(self.args.T[i])+len(self.args.PR[i])+len(self.args.PT[i])+len(self.args.RT[i])+len(self.args.PRT[i])
                # print(ch_length)
                # print(len(self.args.P[i])+len(self.args.R[i])+len(self.args.T[i])+len(self.args.PR[i])+len(self.args.PT[i])+len(self.args.RT[i])+len(self.args.PRT[i]))
            # Resblocks
            flops+= (_input[0]*_input[1])*2*(kernel_size*kernel_size)*self.args.n_feats*2*(ch_length)
            # flops+= (_input[0]*_input[1])*2*(kernel_size*kernel_size)*self.args.n_feats*2*(ch_length) # for doubled EDSR
